...
func enrichFreshClone(dc DoctorCheck) agentEnrichment {
	...
	explanation := fmt.Sprintf("Fresh clone detected: %s. The .beads/ directory exists (committed to git) but the local database has not been recovered yet. Run bd bootstrap to recover existing state safely.", dc.Message)
	...
	if strings.Contains(dc.Message, "sync.git-remote") {
		explanation = fmt.Sprintf("Fresh clone detected: %s. The .beads/ directory exists (committed to git) but the database is not found on the configured server. Run bd bootstrap to recover existing state; if bootstrap cannot find the expected remote automatically, then set sync.git-remote in .beads/config.yaml and rerun bd bootstrap.", dc.Message)
