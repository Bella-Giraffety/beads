...
func (s *DoltStore) getWisp(ctx context.Context, id string) (*types.Issue, error) {
	...
	err := s.withIgnoredTableRepair(ctx, func() error {
		...
		fetched, err := scanIssueFromTable(ctx, s.db, "wisps", id)
		if err != nil {
			return err
		}
		if fetched == nil {
...
func (s *DoltStore) getWispLabels(ctx context.Context, issueID string) ([]string, error) {
	...
	err := s.withIgnoredTableRepair(ctx, func() error {
		...
		if err != nil {
			return wrapQueryError("get wisp labels", err)
		}
		defer rows.Close()
...
func (s *DoltStore) deleteWispBatchTx(ctx context.Context, ids []string) (int, error) {
	...
	err := s.withIgnoredTableRepair(ctx, func() error {
		...
		result, err := tx.ExecContext(ctx,
			...
			args...)
		if err != nil {
			return fmt.Errorf("failed to batch delete wisps: %w", err)
		}
		rowsAffected, _ := result.RowsAffected()
		deleted = int(rowsAffected)

		if err := tx.Commit(); err != nil {
			return fmt.Errorf("failed to commit batch wisp delete: %w", err)
		}
		return nil
	})
	return deleted, err
...
func (s *DoltStore) searchWisps(ctx context.Context, query string, filter types.IssueFilter) ([]*types.Issue, error) {
	...
	err := s.withIgnoredTableRepair(ctx, func() error {
		...
		if err != nil {
			return fmt.Errorf("failed to search wisps: %w", err)
		}
		defer rows.Close()

		issues, err = s.scanWispIDs(ctx, rows)
		return err
	})
	return issues, err
}
...
func (s *DoltStore) getWispsByIDs(ctx context.Context, ids []string) ([]*types.Issue, error) {
	...
	err := s.withIgnoredTableRepair(ctx, func() error {
		...
		for start := 0; start < len(ids); start += queryBatchSize {
			...
			if err != nil {
				return fmt.Errorf("failed to get wisps by IDs: %w", err)
			}
			...
			for queryRows.Next() {
				...
				if err != nil {
					_ = queryRows.Close()
					return wrapScanError("scan wisp", err)
				}
			...
			if err := queryRows.Err(); err != nil {
				_ = queryRows.Close()
				return wrapQueryError("iterate wisps", err)
			}
			_ = queryRows.Close()
		}
